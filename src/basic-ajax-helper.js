class AjaxHelper {
    constructor(options = {}) {
        this.config = {
            messages: {
                ...
                copyContent: 'İçerik Kopyalandı',
                loading: 'Yükleniyor...',
                loadingError: 'Yükleme hatası: ',
                unknownError: 'Bilinmeyen hata',
                warning: 'Uyarı',
                pleaseFillRequiredFields: 'Lütfen gerekli alanları doldurun',
                confirmSubmit: 'Emin misiniz?',
                confirmSubmitText: 'Bu formu göndermek istediğinize emin misiniz?',
                confirmSubmitButton: 'Evet, gönder',
                cancelSubmitButton: 'Vazgeç',
                confirmAction: 'Bu işlemi yapmak istediğinize emin misiniz?',
                confirmButton: 'Evet, devam et',
                cancelButton: 'İptal',
                error: 'Hata',
                processFailed: 'İşlem başarısız oldu.',
                success: 'Başarılı',
                noRecordsSelected: 'Hiçbir kayıt seçilmedi.',
                anyRecordSelected: 'Herhangi bir kayıt seçilmedi.',
                recordsWillBeProcessed: ' kayıt üzerinde işlem yapılacak.',
                selectRecords: 'Lütfen gerekli alanları doldurun',
                fileNotSelected: 'Dosya seçilmedi!',
                operationFailed: 'Bir hata oluştu',
                modalContentEmpty: 'Modal içeriği boş geldi.',
                modalFailed: 'Modal yüklenemedi.',
                tabLoading: 'Yükleniyor...',
                tabFailed: 'Tab yüklenemedi: ',
                saveChanges: 'Değişiklik kaydedilemedi.',
                pleaseEnterInfo: 'Lütfen bilgileri girin',
                send: 'Gönder',
                requiredField: 'Bu alan zorunludur!',
                fieldRequired: ' alanı zorunludur!',
                choose: 'Seçiniz...',
                saved: 'Kaydedildi',
                couldNotSave: 'Kaydedilemedi.',
                sortingUpdated: 'Sıralama güncellendi.',
                sortingNotSaved: 'Sıralama kaydedilemedi.',
                autoSaveFailed: 'Değişiklik kaydedilemedi.',
                sure: 'Emin misiniz?'
            }
        };

        if (options.config && options.config.messages) {
            Object.assign(this.config.messages, options.config.messages);
        }
    ...
    executeCallbacks($element, action) {
        ...
        if (action === 'show') {
            ...
            if (loadUrl && (!loadOnce || !$element.data('loaded'))) {
                ...
                $target.html(`<div class="text-center p-3"><i class="fas fa-spinner fa-spin"></i> ${this.config.messages.loading}</div>`);
                ...
                $.get(loadUrl)
                    ...
                    .fail((xhr) => {
                        $target.html(`<div class="text-danger p-3">${this.config.messages.loadingError}${xhr.responseJSON?.message || this.config.messages.unknownError}</div>`);
                    });
            }
        }
    ...
    checkRemoteLoad($element) {
        ...
        if (this.evaluateSingleCondition(`${selector}:${value}`)) {
            ...
            $.get(url).done((html) => {
                ...
                $element.html(`<div class="text-danger">${this.config.messages.loadingError}</div>`);
            });
        }
    }
    ...
    bindWizard() {
        ...
        $(document).on('click', '[data-next-step]', function(e) {
            ...
            if (nextOn && !self.evaluateSingleCondition(nextOn)) {
                self.notify('warning', self.config.messages.warning, self.config.messages.pleaseFillRequiredFields);
                return;
            }
    ...
    bindCopy() {
        $(document).on('click', '[data-copy]', (e) => {
            ...
            this.notify('success', this.config.messages.copySuccess, this.config.messages.copyContent);
        });
    }
        ...
        $.ajax({
            ...
            success: (res) => {
                ...
                if (res.type && res.message) {
                    if ($form.data('auto-clean')) {
                        ['error', 'warning', 'success'].forEach(type => {
                            const className = $form.data(`${type}-class`);
                            if (className) {
                                $form.find(`.${className}`).removeClass(className);
                            }
                        });
                    }
                    ...
                    ['error', 'warning', 'success'].forEach(type => {
                        const fieldData = res[type];
                        if (!fieldData) return;

                        const className = $form.data(`${type}-class`);
                        const duration = parseInt($form.data(`${type}-duration`)) || 3000;
                        ...
                        fields.forEach(item => {
                            ...
                            if (!firstErrorField) {
                                firstErrorField = $field[0];
                            }

                            if (className) {
                                $field.addClass(className);
                                setTimeout(() => $field.removeClass(className), duration);
                            }
                    ...
                    if (firstErrorField && $form.data('auto-focus')) {
                        firstErrorField.scrollIntoView({ behavior: "smooth", block: "center" });
                        firstErrorField.focus();
                    }

                    if (res.type === 'success' && $form.data('success-element')) {
                        let successBox = $form.find('.form-success-box');
                        if (!successBox.length) {
                            successBox = $('<div class="form-success-box mt-2"></div>').prependTo($form);
                        }
                        successBox.html(`<div class="alert alert-success">${res.message}</div>`);
                    }
                    ...
                    if (res.href || res.redirect) {
                        setTimeout(() => window.location.href = res.href || res.redirect, res.delay || relodDelay || this.reloadDelay);
                    }
            ...
            error: (xhr) => {
                ...
                const msg = xhr.responseJSON?.message || this.config.messages.processFailed;
                this.notify('error', this.config.messages.error, msg);
        ...
        $(document).on('click', '[data-request-with-url]',  function (e) {
            ...
            if (fileAttr) {
                ...
                if (!input || !input.files.length) {
                    self.notify('error', '', self.config.messages.fileNotSelected);
                    resetBtn();
                    return;
                }
            ...
            $.ajax({
                ...
                success: function(data, status, xhr) {
                    ...
                    if (res.type && res.message) {
                        self.notify(res.type, res.title || self.config.messages.success, res.message);
                    }
                ...
                error: function(xhr) {
                    try {
                        ...
                        if (res.type && res.message) {
                            self.notify(res.type, res.title || self.config.messages.error, res.message);
                        }
                        if (res.url) {
                        ...
                        self.notify('error', self.config.messages.error, self.config.messages.operationFailed);
                    }
                    resetBtn();
                }
            });
    ...
    bindConfirmSubmit() {
        $(document).on('click', '[data-confirm-submit]', (e) => {
            ...
            const title = $button.data('confirm-submit-title') || this.config.messages.confirmSubmit;
            const text = $button.data('confirm-submit-text') || this.config.messages.confirmSubmitText;
            const icon = $button.data('confirm-submit-icon') || 'warning';
            const confirm = $button.data('confirm-submit-button') || this.config.messages.confirmSubmitButton;
            const cancel = $button.data('cancel-submit-button') || this.config.messages.cancelSubmitButton;
    ...
    bindAjaxConfirm() {
        $(document).on('click', '[data-ajax-confirm]', (e) => {
            ...
            const noConfirm = $el.data('no-confirm') == 'on';
            const reloadUrl = $el.data('reload-url') || window.location.href;
            const submitLoader = $el.attr('data-submit-loader') !== undefined;

            const title = $el.data('ajax-confirm-title') || this.config.messages.sure;
            const text = $el.data('ajax-confirm-text') || this.config.messages.confirmAction;
            const icon = $el.data('ajax-confirm-icon') || 'warning';
            const confirm = $el.data('ajax-confirm-button') || this.config.messages.confirmButton;
            const cancel = $el.data('ajax-cancel-button') || this.config.messages.cancelButton;
            ...
            const runAjax = () => {
                $.ajax({
                    ...
                    success: (response) => {
                        ...
                        if (response.type) {
                            this.notify(response.type, response.title || this.config.messages.success, response.message || '');
                        }
                    ...
                    error: (xhr) => {
                        resetSubmitter();
                        Swal.fire({
                            title: this.config.messages.error,
                            text: xhr.responseJSON?.message || this.config.messages.processFailed,
                            icon: 'error'
                        });
    ...
    bindAjaxTabs() {
        $(document).on('click', '[data-ajax-tab]', (e) => {
            ...
            $target.html(`<div class="p-3 text-center text-muted">${this.config.messages.loading}</div>`);
            ...
            $.get(url, (html) => {
                ...
                $target.html(
                    `<div class="p-3 text-danger">${this.config.messages.tabFailed}${xhr.responseJSON?.message || this.config.messages.error}</div>`
                );
            });
        });
    }
    ...
    bindAjaxPolling() {
        $('[data-poll]').each((i, el) => {
            const $el = $(el);
            const url = $el.data('url') || $el.attr('href');
            const interval = parseInt($el.data('poll')) || 5000;
    ...
    bindBulkExport() {
        $(document).on('click', '[data-bulk-export]', (e) => {
            ...
            if (ids.length === 0) {
                this.notify('warning', this.config.messages.warning, this.config.messages.noRecordsSelected);
                return;
            }
    ...
    bindBulkActions() {
        ...
        $(document).on('click', '[data-bulk-all]', function () {
            ...
            const isToggled = $btn.data('toggled');
            const originalStates = $btn.data('originalStates');
            const checkboxes = $targets.toArray();
            ...
            function processChunk() {
                ...
                const end = Math.min(start + CHUNK_SIZE, checkboxes.length);

                for (let i = start; i < end; i++) {
                    checkboxes[i].checked = isToggled ? originalStates[i] : true;
                }
        ...
        $(document).on('click', '[data-bulk-action]', (e) => {
            ...
            if (ids.length === 0) {
                this.notify('warning', this.config.messages.warning, this.config.messages.anyRecordSelected);
                return;
            }

            Swal.fire({
                title: this.config.messages.sure,
                text: `${ids.length}${this.config.messages.recordsWillBeProcessed}`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: this.config.messages.confirmButton,
                cancelButtonText: this.config.messages.cancelButton
            }).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        url,
                        ...
                        success: (response) => {
                            this.notify(response.type || 'success', response.title || this.config.messages.success, response.message || '');
                        ...
                        error: (xhr) => {
                            this.notify('error', this.config.messages.error, xhr.responseJSON?.message || this.config.messages.processFailed);
    ...
    bindAjaxPModal() {
        $(document).on('click', '[data-ajax-pmodal]', (e) => {
            ...
            $.ajax({
                ...
                success: (html) => {
                    if (!html || html.trim() === '') {
                        this.notify('error', this.config.messages.error, this.config.messages.modalContentEmpty);
                        return;
                    }
                ...
                error: (xhr) => {
                    this.notify('error', this.config.messages.error, xhr.responseJSON?.message || this.config.messages.modalFailed);
                }
            });
        });
    }
                ...
                onEnd: (evt) => {
                    ...
                    $.ajax({
                        ...
                        success: (res) => {
                            ...
                            this.notify(res.type || 'success', res.title || this.config.messages.success, res.message || this.config.messages.sortingUpdated);
                        ...
                        error: (xhr) => {
                            this.notify('error', this.config.messages.error, xhr.responseJSON?.message || this.config.messages.sortingNotSaved);
                        }
                    });
                }
            });
    ...
    bindAutosave() {
        ...
        $(document).on('input change', '[data-autosave]', (e) => {
            ...
            const url = $form.attr('action') ?? $input.data('action');
            const method = $form.attr('method') || 'POST';
            const delay = $input.data('delay') || 500;
            const fieldName = $input.attr('name');
            const uniq_id = $input.attr('id') ?? 0;
            const fieldValue = $input.val();
            const reloadTarget = $input.data('reload-target');
            const reloadUrl = $input.data('reload-url') || window.location.href;
            const silent = $form.data('silent') == 'on';
            if (silent){
                window.notyf_alert_silent = true;
            }
                ...
                $.ajax({
                    ...
                    error: (xhr) => {
                        ...
                        if (xhr.status === 422 && xhr.responseJSON?.errors) {
                            ...
                            this.notify('error', this.config.messages.error, this.config.messages.autoSaveFailed);
                        }
                    }
                });
            }, delay);
    ...
    bindSwalRequest() {
        $(document).on('click', '[data-swal-request]', (e) => {
            ...
            const title = $el.data('swal-title') || this.config.messages.sure;
            const text = $el.data('swal-text') || this.config.messages.confirmAction;
            const icon = $el.data('swal-icon') || 'warning';
            const confirmButton = $el.data('swal-confirm') || this.config.messages.confirmButton;
            const cancelButton = $el.data('swal-cancel') || this.config.messages.cancelButton;
    ...
    bindSwalInputRequest() {
        $(document).on('click', '[data-swal-input-request]', (e) => {
            ...
            const title = $el.data('swal-title') || this.config.messages.pleaseEnterInfo;
            const text = $el.data('swal-text') || '';
            const confirmButton = $el.data('swal-confirm') || this.config.messages.send;
            const cancelButton = $el.data('swal-cancel') || this.config.messages.cancelButton;
            ...
            const save = () => {
                ...
                $.post(url, data)
                    .done((response) => {
                        ...
                        if (response.type && response.message) {
                            self.notify(response.type, response.title || self.config.messages.success, response.message);
                        }
                    })
                    .fail((xhr) => {
                        $el.removeClass('editing').text(currentValue);
                        self.notify('error', self.config.messages.error, xhr.responseJSON?.message || self.config.messages.couldNotSave);
                    });
            };
    ...
    handleSingleInput($el, inputName, url, method, title, text, confirmButton, cancelButton, extraValues, reloadAfter, reloadTarget, reloadUrl) {
        ...
        Swal.fire({
            ...
            inputValidator: (value) => {
                if (required && !value) {
                    return this.config.messages.requiredField;
                }
            }
        }).then((result) => {
            if (result.isConfirmed) {
    ...
    handleMultipleInputs($el, layout, url, method, title, text, confirmButton, cancelButton, extraValues, reloadAfter, reloadTarget, reloadUrl) {
        ...
        fields.forEach(field => {
            ...
            if (field.type === 'textarea') {
                ...
                formHtml += `<option value="">${this.config.messages.choose}</option>`;
                if (options) {
        ...
        Swal.fire({
            ...
            preConfirm: () => {
                ...
                fields.forEach(field => {
                    ...
                    if (isRequired && !value) {
                        Swal.showValidationMessage(`${field.placeholder || field.name}${this.config.messages.fieldRequired}`);
                        hasError = true;
                        return false;
                    }
